...
public class EasyEnemy {
    ...
    private int SHIP_SIZE = 64;
    public EasyEnemy(){
        x=0;y=0;hp=3;
        this.sprite = new Sprite(new Texture("ship-blue.png"));
        this.sprite.setSize(this.SHIP_SIZE,this.SHIP_SIZE*(this.sprite.getHeight()/this.sprite.getWidth())); //ALSO GET SHIP RESOLUTION IN CASE THE SPRITE IS NOT A SQUARE
    ...
    public void move(float pX, float pY){
        ...
        if(Math.sqrt(Math.pow(pX-this.x,2)+Math.pow(pY-this.y,2))>150){
            //First stage: Running directly at the player
            System.out.println(true);
            this.theta = (float) Math.atan2(pX-this.x,pY-this.y);
